namespace draw
{
	...
	pixel(PixelBuffer* buffer, i32 x, i32 y, u32 color)
	{
		...
		auto pixel = (u8*)(buffer->memory) + offset;
		*(u32*)pixel = color;
	}
	...
	{
		auto i_min_x = round_to_i32(rect.min.x * buffer->pixels_per_world_unit);
		auto i_min_y = round_to_i32(rect.min.y * buffer->pixels_per_world_unit);
		auto i_max_x = round_to_i32(rect.max.x * buffer->pixels_per_world_unit);
		auto i_max_y = round_to_i32(rect.max.y * buffer->pixels_per_world_unit);
